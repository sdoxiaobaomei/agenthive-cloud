      ...
      <div class="content-left">
        <el-card class="agent-overview" shadow="hover" v-loading="loading">
          ...
          <div v-else class="agent-mini-list">
            ...
            >
              <AgentAvatar :role="agent.role" :status="agent.status as any" :size="40" />
              <div class="mini-info">
                <div class="mini-name">{{ agent.name }}</div>
                <div class="mini-status" :class="agent.status">
                  {{ formatStatus(agent.status) }}
                  <span v-if="(agent as any).currentTask" class="mini-task">
                    · {{ truncate((agent as any).currentTask?.title, 20) }}
                  </span>
                </div>
              </div>
              <div class="mini-progress" v-if="(agent as any).currentTask && (agent as any).currentTask?.progress > 0">
                <el-progress 
                  :percentage="(agent as any).currentTask?.progress || 0" 
                  :stroke-width="3"
                  :show-text="false"
                />
              </div>
