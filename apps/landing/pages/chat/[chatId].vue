...
onMounted(async () => {
  // Validate chatId
  if (!chatId.value) {
    sessionError.value = 'Invalid chat ID'
    return
  }
  ...
  try {
    ...
    const res = await chatApi.getSession(chatId.value)
    if (!res.success) {
      sessionError.value = res.message || 'Chat session not found'
      return
    }

    // Set current conversation so projectId is available for file API
    ...
    sessionError.value = err.message || 'Failed to load chat session'
    return
  }
...
async function loadFileContent(path: string) {
  try {
    const projectId = chatStore.currentConversation?.projectId
    if (!projectId) {
      fileContent.value = '// No project associated with this chat'
      return
    }
    const { code: codeApi } = useApi()
    const response = await codeApi.getFile(projectId, path)
