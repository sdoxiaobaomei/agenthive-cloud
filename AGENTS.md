...
- **对象存储**: MinIO
- **可观测性**: OpenTelemetry + Grafana LGTM (Tempo/Loki/Prometheus/Grafana) + Beyla (eBPF)
- **反向代理**: Nginx
...
AgentHive Cloud 采用企业级 OpenTelemetry 可观测架构：

| 层级 | 技术 | 职责 |
|------|------|------|
| 信号采集 | OpenTelemetry SDK (Node.js) + Beyla (eBPF) | 自动/无侵入采集 Traces/Metrics/Logs |
| 信号汇聚 | OTel Collector (Gateway + Agent) | Batch、采样、过滤、路由 |
| 追踪存储 | Grafana Tempo (S3/MinIO 后端) | 分布式追踪，TraceQL 查询 |
| 指标存储 | Prometheus | 时序指标，PromQL 查询 |
| 日志存储 | Grafana Loki | 日志聚合，LogQL 查询 |
| 可视化 | Grafana | 统一 Dashboard、告警、Trace-Log-Metric 联动 |

### AI 业务可观测

| 观测对象 | Span 名称 | 关键属性 |
|----------|-----------|----------|
| LLM 调用 | `agenthive.llm.completion` | `llm.provider`, `llm.model`, `llm.tokens.*`, `llm.cost_usd` |
| Agent 任务 | `agenthive.runtime.task` | `agent.id`, `task.id`, `task.type` |
| QueryLoop | `agenthive.query_loop.execute` | `query_loop.iteration`, `llm.tokens.total` |
| Tool 执行 | `agenthive.tool.execute` | `tool.name`, `tool.duration_ms` |
| WebSocket | `agenthive.websocket.*` | `websocket.event_type`, `websocket.connection_id` |
...
| 工作区数量 | < 10 | > 15 |
| 文件搜索响应 | < 1秒 | > 3秒 |
| OTel Collector CPU | < 50% | > 70% |
| OTel Collector 内存 | < 256MB | > 512MB |
