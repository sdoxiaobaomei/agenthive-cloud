...
import type { AgentStatus, Message, CodeFile, TerminalOutput, MessageType } from '@agenthive/types'
...
export const useWebSocketStore = defineStore('websocket', () => {
  ...
  // 模拟接收 Agent 状态变更
  const simulateAgentStateChange = (data: {
    agentId: string
  ...
  // 模拟接收任务进度
  const simulateTaskProgress = (data: { taskId: string; progress: number }) => {
    const taskStore = useTaskStore()
    taskStore.updateTaskProgress(data.taskId, data.progress)
  }
  ...
  return {
    // State
    socket: ref(null), // 保持兼容，但始终为 null
    status,
    reconnectAttempts,
    lastError,
    currentCode,
