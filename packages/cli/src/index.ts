...
import { SelfHostedRuntimeAdapter } from '../../../agents/adapters/runtime-adapter.js'
import { initStagingBranch, applyTicketToStaging, syncStagingToMaster } from '../../../agents/tools/git-staging.js'
import type { ExecutionResult } from '../../workflow-engine/src/types.js'
...
async function main() {
  ...
  if (!requirement && !resumeSessionId) {
    console.error('Usage: npx agenthive "你的需求"')
    console.error('   or: npx agenthive --resume <session-id>')
    process.exit(1)
  }
