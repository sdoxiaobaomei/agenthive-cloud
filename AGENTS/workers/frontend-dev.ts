...
const promptPath = path.join(__dirname, '../shared/prompts/system-frontend-dev.md')
const ticketPath = process.argv[2]

if (!ticketPath) {
  console.error('Usage: tsx workers/frontend-dev.ts <ticket.json>')
  process.exit(1)
}
...
runWorker(ticketPath, promptPath, async (client, ticket, workspaceRepo, systemPrompt) => {
  const res = await client.chatJson<FEResult>([
  ...
  for (const file of allFiles) {
    // file.path may be relative like "apps/web/src/..." or absolute-like
